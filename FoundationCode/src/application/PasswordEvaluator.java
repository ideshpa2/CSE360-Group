...
public class PasswordEvaluator {
	...
	public static boolean foundOtherChar = true; 
	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			if (currentChar >= 'A' && currentChar <= 'Z') {
				...
				// Changes were made to some of the special characters allowed within the password
 			} else if ("~'!@#$%^&*()_-+{}[]:,.?/".indexOf(currentChar) >= 0) {
				System.out.println("Special character found");
				foundSpecialChar = true;
				// otherChar boolean value was integrated
			} else {
		...
		String errMessage = "";
		if (!foundUpperCase)
			errMessage += "Missing an upper case; ";
		
		if (!foundLowerCase)
			errMessage += "Missing a lower case; ";
		
		if (!foundNumericDigit)
			errMessage += "Missing a numeric digits; ";
			
		if (!foundSpecialChar)
			errMessage += "Missing an special character; ";
			
		if (!foundLongEnough)
			errMessage += "Not long enough; ";
		
		// otherChar boolean is integrated into the code for catching invalid characters

		if (foundOtherChar) 
			errMessage += "Invalid characters were used; "; 		
		if (errMessage == "")
			return "";
